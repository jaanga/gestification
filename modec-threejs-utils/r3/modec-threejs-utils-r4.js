...
// TBD: Name? MDU - mition detector utilities? Namespace & objectification
	...
	MDU.headsUpClose = '<div style="margin: 0; text-align: right;">[<i>make fist to close</i>]</div>';
	...
	'';
	MDU.headsUpAbout = MDU.headsUpClose +
		'<h1>About</h1>' +
	...
	MDU.setHeadsUp = function() {
		...
		MDU.headsUp.style.cssText = 'background-color: #eee; border-radius: 8px; color: #333; left: 0; ' +
			'margin: auto; opacity: 0.85; padding: 10px 20px 10px 20px; ' +
			'position: absolute; text-align: left; right: 0; top: -8px; min-width: 280px; width: 30%;';			
		MDU.headsUp.onclick = toggleHeadsUp;
		MDU.headsUp.innerHTML = MDU.headsUpAbout;	
	};
	
	MDU.utilsDetectCollision = function ( pointer, pointerTip) {
		var intersected = MDU.intersected;
// Big thanks to Lee Stemkoski for the following code
	...
	// hands	
		...
		mesh.doIt = function() { MDU.headsUp.innerHTML = '<p style="margin: 0; text-align: right;"><i>click here to close...</i></p><h2>Switch hands</h2>Coming soon...' ; }
		...
		// mesh.castShadow = true;
		// mesh.receiveShadow = true;
		// mesh.doIt = function() { MDU.headsUp.style.display = ''; MDU.headsUp.innerHTML = '<p style="margin: 0; text-align: right;"><i>click here to close...</i></p><h2>Switch hands</h2>Coming soon...' ; }
		MDU.collidableMeshList.push( mesh );
		toolbar.add( mesh );
...
// help-sphere
		...
		// mesh.castShadow = true;
		mesh.receiveShadow = true;
		mesh.doIt = function() { MDU.headsUp.style.display = ''; MDU.headsUp.innerHTML = MDU.headsUpHelp; }
		MDU.collidableMeshList.push( mesh );
		toolbar.add( mesh );
...
// info-sphere
		...
		// mesh.castShadow = true;
		mesh.receiveShadow = true;
		mesh.doIt = function() { MDU.headsUp.style.display = ''; MDU.headsUp.innerHTML = MDU.headsUpAbout; }
		MDU.collidableMeshList.push( mesh );
		toolbar.add( mesh );
...
// pointer
	...
	MDU.resetCamera = function ( parameters ) {
		...
		var camY = pars.camY ? pars.camY : 100;
		var camZ = pars.camZ ? pars.camZ : 750;
